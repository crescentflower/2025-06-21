    ...
    >
      ...
      >
        <div class="kline-container" ref="klineContainer">
          <!-- 时间筛选控件 -->
          <div class="time-filter" v-if="showTimeFilter && (state === STATE_ARRAY[0] || state === STATE_ARRAY[1])">
            <div class="filter-control">
              <label>开始时间:</label>
              <input type="datetime-local" v-model="startDate" @change="applyTimeFilter">
            </div>
            <div class="filter-control">
              <label>结束时间:</label>
              <input type="datetime-local" v-model="endDate" @change="applyTimeFilter">
            </div>
            <div class="filter-control">
              <Button @click="resetTimeFilter" size="small">重置筛选</Button>
            </div>
          </div>
      ...
      for (const record of records) {
        ...
        for (const fieldId in fields) {
          ...
          if (requiredFields.includes(fieldName) || 
            ...
            kPoint[fieldName] = fields[fieldId];
          }
        }
...
const applyTimeFilter = () => {
  if (!startDate.value || !endDate.value) {
    filteredKlineData.value = [...klineData.value];
    renderKLineChart();
    return;
  }
  ...
  filteredKlineData.value = klineData.value.filter(item => {
...
const handleMouseMove = (event: MouseEvent) => {
  ...
  const index = Math.floor((mouseX - margin.left) / (actualBarWidth + barSpacing));
...
const saveConfig = async () => {
  ...
  const config = {
    customConfig: {
      ...
      startDate: startDate.value,  // 保存开始时间
      endDate: endDate.value,      // 保存结束时间
    },
  };
